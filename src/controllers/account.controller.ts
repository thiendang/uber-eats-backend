...
export const initOwnerAccount = async () => {
  ...
  if (accountCount === 0) {
    ...
    console.log(
      chalk.bgCyan(
        `Successfully created owner account: ${envConfig.INITIAL_EMAIL_OWNER}|${envConfig.INITIAL_PASSWORD_OWNER}`
      )
    )
  }
}
  ...
  try {
    ...
    if (isPrismaClientKnownRequestError(error)) {
      if (error.code === PrismaErrorCode.UniqueConstraintViolation) {
        throw new EntityError([{ field: 'email', message: 'Email is existed' }])
      }
    }
    throw error
  }
...
export const updateEmployeeAccount = async (accountId: number, body: UpdateEmployeeAccountBodyType) => {
  try {
    ...
    if (!oldAccount) {
      throw new EntityError([{ field: 'email', message: 'The account you are trying to update no longer exists!' }])
    }
