...
export const createOrdersController = async (orderHandlerId: number, body: CreateOrdersBodyType) => {
  ...
  if (guest.tableNumber === null) {
    throw new Error('The table associated with this guest has been deleted, please select another guest!')
  }
  ...
  if (table.status === TableStatus.Hidden) {
    throw new Error(`The table ${table.number} associated with the guest has been hidden, please select another guest!`)
  }
...
// Controller pays bills based on guestId
export const payOrdersController = async ({ guestId, orderHandlerId }: { guestId: number; orderHandlerId: number }) => {
  ...
  if (orders.length === 0) {
    throw new Error('No bills to pay')
  }
